...
public class WeaponDragCtrl : MonoBehaviour
{

    public Transform Img;   // 빈 이미지 객체.

    private Image EmptyImg; // 빈 이미지.
    private WeaponSlotCtrl slot;      // 현재 슬롯에 스크립트
    ...
    {
        // 현재 슬롯의 스크립트를 가져온다.
        slot = GetComponent<WeaponSlotCtrl>();
    ...
    public void DragEnd()
    {
        ...
        Iv.WeaponSwap1(slot, Img.transform.position);
        //slot = null;
    }
    ...
    public void Up()
    {
        ...
        // 슬롯의 아이템 이미지를 복구 시킨다.
        slot.UpdateInfo(true, slot.weaponSlot.Peek().DefaultImg);
    }
}
