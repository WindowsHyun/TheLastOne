...
bool UnityGeom::IsPointNearNavMesh(float x, float z)
{
	if (!m_isLoaded)
	{
		return false;
...
bool UnityGeom::FindStraightPath(const UnityVertex& start, const UnityVertex& end, std::vector<UnityVertex>& path)
{
	dtPolyRef startRef, endRef;
	return FindStraightPath(start, end, path, startRef, endRef);
}

bool UnityGeom::FindStraightPath(const UnityVertex& start, const UnityVertex& end, std::vector<UnityVertex>& path, dtPolyRef& startPolyRef, dtPolyRef& endPolyRef)
{
	path.clear();
