...
bool dtCreateNavMeshData(dtNavMeshCreateParams* params, unsigned char** outData, int* outDataSize)
{
	if (params->nvp > DT_VERTS_PER_POLYGON) {
		std::cout << "DT_VERTS_PER_POLYGON error!" << std::endl;
		return false;
	}
	if (params->vertCount >= 0xffff) {
		std::cout << "vertCount error! Count : "  << params->vertCount << std::endl;
		return false;
	}
	if (!params->vertCount || !params->verts) {
		std::cout << "!params->vertCount || !params->verts" << std::endl;
		return false;
	}
	if (!params->polyCount || !params->polys) {
		std::cout << "!params->polyCount || !params->polys" << std::endl;
		return false;
	}
	...
	if (params->offMeshConCount > 0)
	{
		...
		if (!offMeshConClass) {
			std::cout << "!offMeshConClass" << std::endl;
			return false;
		}
	...
	if (!data)
	{
		...
		std::cout << "!offMeshConClass error" << std::endl;
		return false;
	}
