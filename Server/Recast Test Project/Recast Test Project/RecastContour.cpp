					 ...
					 rcContourSet& cset, const int buildFlags)
{
	...
	if (!flags)
	{
		std::cout << RC_LOG_ERROR << "rcBuildContours: Out of memory 'flags' (" << chf.spanCount << ")." << std::endl;
		return false;
	}
		...
		for (int x = 0; x < w; ++x)
		{
			...
			for (int i = (int)c.index, ni = (int)(c.index+c.count); i < ni; ++i)
			{
				...
				if (simplified.size()/4 >= 3)
				{
					...
					if (!cont->verts)
					{
						std::cout << RC_LOG_ERROR << "rcBuildContours: Out of memory 'verts' (" << cont->nrverts << ")." << std::endl;
					...
					if (!cont->rverts)
					{
						std::cout << RC_LOG_ERROR << "rcBuildContours: Out of memory 'rverts' (" << cont->nrverts << ")." << std::endl;
						return false;
					}
...
/*					cont->cx = cont->cy = cont->cz = 0;
	...
	for (int i = 0; i < cset.nconts; ++i)
	{
		...
		if (calcAreaOfPolygon2D(cont.verts, cont.nverts) < 0)
		{
			...
			if (mergeIdx == -1)
			{
				std::cout << RC_LOG_WARNING << "rcBuildContours: Could not find merge target for bad contour " << i << std::endl;
			...
			else
			{
				...
				if (ia == -1 || ib == -1)
				{
					std::cout << RC_LOG_WARNING << "rcBuildContours: Failed to find merge points for" << i << " and " << mergeIdx<< std::endl;
					//ctx->log(RC_LOG_WARNING, "rcBuildContours: Failed to find merge points for %d and %d.", i, mergeIdx);
					continue;
				}
				if (!mergeContours(mcont, cont, ia, ib))
				{
					std::cout << RC_LOG_WARNING << "rcBuildContours: Failed to merge contours " << i << " and " << mergeIdx << std::endl;
