...
void Sample::handleRender()
{
	...
	// Draw mesh
	//duDebugDrawTriMesh(&dd, m_geom->getMesh()->getVerts(), m_geom->getMesh()->getVertCount(), m_geom->getMesh()->getTris(), m_geom->getMesh()->getNormals(), m_geom->getMesh()->getTriCount(), 0, 1.0f);
	// Draw bounds
...
void Sample::handleCommonSettings()
{
	if (m_geom)
	{
		...
		char text[64];
		std::cout << "Voxels" << gw << " x "  << gh << std::endl;
	}
}
