...
bool InputGeom::loadMesh(const char* filepath)
{
	...
	if (!m_mesh)
	{
		std::cout << RC_LOG_ERROR << "loadMesh: Out of memory 'm_mesh'." << std::endl;
		return false;
	}
	if (!m_mesh->load(filepath))
	{
		std::cout << RC_LOG_ERROR << "buildTiledNavigation: Could not load '%s" << filepath << std::endl;
		return false;
	}
	...
	if (!m_chunkyMesh)
	{
		std::cout << RC_LOG_ERROR << "buildTiledNavigation: Out of memory 'm_chunkyMesh'." << std::endl;
		return false;
	}
	if (!rcCreateChunkyTriMesh(m_mesh->getVerts(), m_mesh->getTris(), m_mesh->getTriCount(), 256, m_chunkyMesh))
	{
		std::cout << RC_LOG_ERROR << "buildTiledNavigation: Failed to build chunky mesh." << std::endl;
		return false;
	}		
