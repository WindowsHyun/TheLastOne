...
static int addEdge( int* edges, int& nedges, const int maxEdges, int s, int t, int l, int r)
{
	if (nedges >= maxEdges)
	{
		std::cout << RC_LOG_ERROR << "addEdge: Too many edges" << std::endl;
						 ...
						 rcIntArray& tris, rcIntArray& edges)
{
	...
	for (int i = 0; i < tris.size()/4; ++i)
	{
		...
		{
			std::cout << RC_LOG_WARNING << "delaunayHull: Removing dangling face " << i << std::endl;
							...
							rcIntArray& edges, rcIntArray& samples)
{
	...
	if (tris.size() == 0)
	{
		...
		std::cout << RC_LOG_WARNING << "buildPolyDetail: Could not triangulate polygon, adding default data." << std::endl;
	...
	if (ntris > MAX_TRIS)
	{
		...
		std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Shrinking triangle count from" << ntris << std::endl;
						   ...
						   rcPolyMeshDetail& dmesh)
{
	...
	if (!bounds)
	{
		std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Out of memory 'bounds'" << mesh.npolys * 4 << std::endl;
	...
	if (!poly)
	{
		std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Out of memory 'poly' " << nvp * 3 << std::endl;
	...
	if (!hp.data)
	{
		std::cout << RC_LOG_ERROR <<"rcBuildPolyMeshDetail: Out of memory 'hp.data'" << maxhw * maxhh << std::endl;
	...
	if (!dmesh.meshes)
	{
		std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Out of memory 'dmesh.meshes'" << dmesh.nmeshes * 4 << std::endl;
	...
	if (!dmesh.verts)
	{
		std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Out of memory 'dmesh.verts' " << vcap * 3 << std::endl;
	...
	if (!dmesh.tris)
	{
		std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Out of memory 'dmesh.tris' " << tcap * 4 << std::endl;
	...
	for (int i = 0; i < mesh.npolys; ++i)
	{
		...
		if (dmesh.nverts+nverts > vcap)
		{
			...
			if (!newv)
			{
				std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Out of memory 'newv' " << vcap * 3 << std::endl;
		...
		if (dmesh.ntris+ntris > tcap)
		{
			...
			if (!newt)
			{
				std::cout << RC_LOG_ERROR << "rcBuildPolyMeshDetail: Out of memory 'newt' " << tcap * 4 << std::endl;
